...
sub download : Path('download') : Args(0) {
    ...
    while ( my $report = $problems->next ) {
        my $user_id = $report->inspection_log_entry->user->id;
        $inspectors->{$user_id} ||= [];
        push @{ $inspectors->{$user_id} }, $report;
    }
    ...
    my $i = 0;
    foreach my $inspections (values %$inspectors) {
        my $initials = @$inspections[0]->inspection_log_entry->user->get_extra_metadata('initials');
